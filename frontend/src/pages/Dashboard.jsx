...
import { LogOut, Lock, Globe, AlertCircle, Loader2, GitBranch, Star, Eye, RefreshCw } from 'lucide-react';
import axios from 'axios';

function Dashboard() {
  const { user, token, logout } = useAuth();  // ✅ use token not idToken
  ...
  useEffect(() => {
    ...
    if (token) fetchRepos(); // ✅ only fetch when token is ready
  ...
  const generateReadme = async (repo) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/repos/generate-readme-private`,
        { owner, repoName },
      ...
      let errorMessage = error.response?.data?.message || error.message || 'Failed to generate README';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-white text-lg font-medium">Loading your repositories...</p>
          <p className="text-slate-400 text-sm mt-2">Please wait while we fetch your GitHub data</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center relative overflow-hidden px-4">
        ...
        <div className="text-center text-white max-w-md relative z-10">
          ...
          <p className="text-red-400 mb-8 bg-red-500/10 border border-red-500/20 rounded-lg p-4">{error}</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6 sm:p-8 relative overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-purple-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-blue-500/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
      ...
      <header className="flex justify-between items-center mb-12 relative z-10">
        ...
        <button
          onClick={handleLogout}
          className="bg-gradient-to-r from-slate-800 to-slate-700 hover:from-slate-700 hover:to-slate-600 text-white font-semibold py-3 px-6 rounded-xl flex items-center transition-all duration-300 group border border-slate-600/50 hover:border-slate-500/50 shadow-lg hover:shadow-xl relative overflow-hidden"
        >
          <div className="absolute inset-0 bg-gradient-to-r from-red-600/0 via-red-600/0 to-red-600/0 group-hover:from-red-600/10 group-hover:via-red-600/5 group-hover:to-red-600/10 transition-all duration-300"></div>
          <LogOut className="mr-3 w-5 h-5 group-hover:scale-110 transition-transform duration-200" />
          <span className="relative z-10">Logout</span>
